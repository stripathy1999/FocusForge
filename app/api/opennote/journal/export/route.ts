...
// Import Opennote functions - need to import from backend
// Since backend/lib might not be accessible, we'll need to call backend API
// Or copy the markdown generation logic here
...
export async function POST(request: Request) {
  try {
    ...
    if (backendUrl) {
      // External backend
      fetchUrl = `${backendUrl}/api/opennote/journal/export`;
    } else {
      // Try backend route - construct absolute URL from request
